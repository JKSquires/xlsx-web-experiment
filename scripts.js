...
const sheet_area = document.getElementById("sheet_area");
...
/* Displays all sheet information.
Globals Used:
- sheet_area
- sheets
Side-Effects:
...
function displaySheets() {
	...
	for (let sheet_i = 0; sheet_i < sheets.length; sheet_i++) {
		const sheet = sheets[sheet_i];

		const sheet_element = document.createElement("div");

		const sheet_label = document.createElement("h3");
		sheet_label.innerText = sheet.name;

		const sheet_name_input = document.createElement("textarea");
		sheet_name_input.innerText = sheet.name;

		const change_sheet_name = document.createElement("button");
		change_sheet_name.innerText = "Change Sheet Name";
		change_sheet_name.addEventListener("click", () => {
			sheet.name = sheet_name_input.value;
			displaySheets();
		});

		const sheet_textarea = document.createElement("textarea");
		sheet_textarea.rows = 35;
		sheet_textarea.style.width = "100%";
		sheet_textarea.value = sheet.value;
		sheet_textarea.addEventListener("input", () => {
			sheet.value = sheet_textarea.value;
		});

		const remove_sheet = document.createElement("button");
		remove_sheet.innerHTML = "Remove Sheet: <strong>" + sheet.name + "</strong>";
		remove_sheet.style.backgroundColor = "#f66";
		remove_sheet.addEventListener("click", () => {
			sheets = sheets.filter((element, index) => index !== sheet_i);
			displaySheets();
		});

		sheet_element.append(
			sheet_label,
			sheet_name_input,
			change_sheet_name,
		...
		sheet_area.append(sheet_element);
	}
}

/* Adds a default sheet to the list of sheets.
Globals Used:
- sheets
Side-Effects:
...
/* Creates an XLSX file based on user-inputted data and downloads it.
Globals Used:
- sheets
- file_name
Side-Effects:
...
sheets.push(createSheet(1, '<sheetData>\n  <row r="1">\n    <c r="A1" t="inlineStr">\n      <is><t>Hello, World!</t></is>\n    </c>\n  </row>\n  <row r="2">\n    <c r="A2">\n      <v>123</v>\n    </c>\n  </row>\n</sheetData>\n\n<mergeCells count="1">\n  <mergeCell ref="A1:B1"/>\n</mergeCells>'));
displaySheets();
